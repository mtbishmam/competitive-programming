...
int main()
{
    ...
    for (int Ti = 1; Ti <= T; Ti++) {
        ...
        ll dp[n][2];
        bool vis[n][2];
        for (int i = 0; i < n; i++) dp[i][0] = dp[i][1] = 0, vis[i][0] = vis[i][1] = 0;
        function<ll(int, bool, int)> f = [&](int i, bool flag, int prev) {
            if (i == n) return 1ll * prev;
            if (vis[i][flag]) return dp[i][flag];
            vis[i][flag] = 1;
            ll& ret = dp[i][flag];
            ret = prev + f(i + 1, 0, a[i]);
            ret = max(ret, -prev + f(i + 1, 1, -a[i]));
            return ret;
            };
        cout << f(1, 0, a[0]) << endl;
    }
