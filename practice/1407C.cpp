...
void solve(int cs) {
    int n; cin >> n;
    vi ans(n+1); set<int> s, si; rep(i,1,n+1) s.insert(i), si.insert(i);
    ...
    while (sz(si) > 1) {
        int i, j;
        auto it = si.begin(); i = *it; si.erase(it);
        auto jt = si.begin(); j = *jt; si.erase(jt);
        int x = ask(i, j);
        int y = ask(j, i);
        if (x < y) swap(x, y), swap(i, j), swap(it, jt);
        ans[i] = x;
        si.insert(j);
        s.erase(x);
    }
    ans[*si.begin()] = *s.begin();
    cout << "! ";
