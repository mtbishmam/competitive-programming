...
struct subtree_add_subtree_sum_top_tree_node : public wala::top_tree_node_base<subtree_add_subtree_sum_top_tree_node> {
...
int main() {
	...
	int N, Q; std::cin >> N >> Q;
	std::vector<subtree_add_subtree_sum_top_tree_node> verts(N);
	for (int i = 0; i < N; i++) {
		std::cin >> verts[i].val;
		verts[i].is_vert = verts[i].is_path = true;
		verts[i].update();
	}
	std::vector<subtree_add_subtree_sum_top_tree_node> edges(N-1);
	for (int e = 0; e < N-1; e++) {
		int u, v; std::cin >> u >> v;
